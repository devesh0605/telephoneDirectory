...
class Ui_Form(object):
    ...
    def retranslateUi(self, Form):
        ...
        self.label.setText(_translate("Form", "                                                          Search by first name"))
        self.sef.setPlaceholderText(_translate("Form", "Enter first name"))
    ...
    def display(self):
        ...
        if x.isalpha():
            import retrieve
            if retrieve.byfirstname(x) == 0:
                self.fn.setPlainText('Sorry')
                self.ln.setPlainText('No')
                self.co.setPlainText('Entries')
                self.ad.setPlainText('Found')
            else:
                self.fn.setPlainText(retrieve.byfirstname(x)[0])
                self.ln.setPlainText(retrieve.byfirstname(x)[1])
                self.co.setPlainText(retrieve.byfirstname(x)[2])
                self.ad.setPlainText(retrieve.byfirstname(x)[3])
        else:
            self.fn.setPlainText('Enter')
            self.ln.setPlainText('Valid')
            self.co.setPlainText('First')
